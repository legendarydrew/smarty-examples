<?php
/**
 * Smarty {foley_field_error} function plugin
 *
 * Type:     function<br>
 * Name:     foley_field_error<br>
...
function smarty_block_foley_field_error($params, $content, $template, &$repeat)
{
  ...
  // Make sure this tag is not inside another foley_field_error tag.
  if ($repeat)
  {
    if ($nesting > 0)
    {
      $smarty->trigger_error('foley_field_error: tag should not be nested.');
      return;
    }
    else
    {
  ...
  {
    $smarty->trigger_error('foley_field_error: missing \'table\' parameter');
    return;
  }
  if (!isset($params['field']))
  {
    $smarty->trigger_error('foley_field_error: missing \'field\' parameter');
    return;
  }
  ...
  if ($repeat)
  {
    ...
    // between the foley_field_error tags. We can do this by temporarily unassigning $message from
    // Smarty.
    ...
    // Add the message to the contents of the foley_field_error tag, wrapped in a SPAN tag with the
    // error class.
