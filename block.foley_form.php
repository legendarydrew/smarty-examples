...
function smarty_block_foley_form($params, $content, $template, &$repeat)
{

	static $form_tags;

	// First check if this foley_form tag is nested in another, which shouldn't happen.
	if ($form_tags > 1)
	{
		trigger_error('foley_form: foley_form tags cannot be nested');
		return;
	}

	// If we've just opened the tag we increment form_tags, otherwise we create a form and process the
	// contents. form_tags will tell us if we've encountered a previous [unclosed] foley_form tag.
	if ($repeat)
	{
		$form_tags++;
	}
	else
	{
		...
		// Return the form and 'close' the foley_form tag.
